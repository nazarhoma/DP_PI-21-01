...
try {
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            // Переконуємося, що рейтинг в межах від 1 до 5
            if ($rating < 1) $rating = 1;
            if ($rating > 5) $rating = 5;

            // Кількість студентів
            $students = intval($row['students_count']);
            if ($students < 1) $students = 100; // Мінімальне значення, якщо немає студентів
        ...
        if ($result_alt->num_rows > 0) {
            while ($row = $result_alt->fetch_assoc()) {
                ...
                $instructors[] = array(
                    ...
                    'name' => $name,
                    'role' => "Інструктор",
                    'rating' => 4.5, // Значення за замовчуванням
                    'students' => 100, // Значення за замовчуванням
                    'image' => $image
                );
            }
            $response['success'] = true;
