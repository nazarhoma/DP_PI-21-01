...
try {
    ...
    if ($result->num_rows > 0) {
        ...
        $update_sql = "UPDATE user_progress SET completed = 1 WHERE user_id = ? AND resource_id = ?";
        ...
        // Записа немає, створюємо новий
        $insert_sql = "INSERT INTO user_progress (user_id, resource_id, completed, started_at) VALUES (?, ?, 1, NOW())";
        
        $stmt = $conn->prepare($insert_sql);
        if (!$stmt) {
            throw new Exception("Помилка підготовки запиту вставки: " . $conn->error);
        }
        
        $stmt->bind_param('ii', $user_id, $resource_id);
        $stmt->execute();
        
        $debug_info['action'] = 'insert';
        $debug_info['insert_id'] = $stmt->insert_id;
    }
    ...
    if ($course_id > 0) {
        ...
        if ($total_resources > 0) {
            ...
            // Якщо прогрес 100%, змінюємо статус на "completed"
            if ($progress >= 100) {
                $update_status_sql = "
                    UPDATE course_enrollments 
                ...
                $debug_info['status_completed_update'] = $stmt->affected_rows;
            }
        }
    }
